...
def cor_teor_mn(teor):
    if teor < 5: return '#00008B'
    elif teor < 10: return '#00CED1'
    elif teor < 15: return 'green'
    elif teor < 20: return 'yellow'
    elif teor < 30: return 'orange'
    else: return '#8B0000'
...
def atualizar_conteudo(furo_selecionado, aba):
    if aba == 'barra':
        ...
        fig = go.Figure(go.Bar(
            x=dff['MN'], y=profundidades, orientation='h',
            marker=dict(color=dff['cor']),
            text=[f"{v:.2f}%" for v in dff['MN']], textposition='auto'
        ))

        fig.update_layout(
            xaxis_title="Teor de Manganês (%)", yaxis_title="Profundidade",
        ...
        return [
            ...
            html.Div([
                ...
                html.P(f"Máximo teor registrado: {dff['MN'].max():.2f}%"),
                html.P(f"Mínimo teor registrado: {dff['MN'].min():.2f}%")
        ...
        return [dcc.Graph(figure=fig),
                html.Ul([
                    html.Li(f"{row.LOCAL}: {row.FURO} furos | Média Mn: {row.MN:.2f}%")
        ...
        fig.update_layout(
            xaxis_title="Teor de Manganês (%)", yaxis_title="Frequência",
        ...
        return html.Div([
            html.Label("Valor de corte (%):"),
            dcc.Input(id="cutoff-input", type="number", value=10, step=0.1),
            html.Div(id="cutoff-output", className="mt-3 text-info")
        ], className="p-3")

    elif aba == 'coleta':
        try:
            with open('assets/mapa_amostras_solo.html', 'r', encoding='utf-8') as f:
                mapa_html = f.read()
            return html.Iframe(srcDoc=mapa_html, width="100%", height="600")
        except Exception as e:
            return html.Div(f"Erro ao carregar mapa: {e}", className="text-danger")
...
def atualizar_cutoff(corte):
    ...
    for local, grupo in df.groupby('LOCAL'):
        ...
        saidas.append(html.Div([
            html.P([
                html.Strong(f"{local}: "),
                f"{total} amostras → {len(filtrado)} acima do cutoff {corte:.1f}%",
                f" | Mn: {media_mn:.2f}%, max: {max_mn:.2f}%, min: {min_mn:.2f}%",
                f" | Fe: {media_fe:.2f}%, Mn/Fe: {mn_fe:.2f}"
            ])
        ]))
    return saidas
